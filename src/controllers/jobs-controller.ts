...
export class JobController {
  ...
  async setPayment (req: RequestCustom, res: Response) {
    ...
    const client: IProfile | null = (await Profile.findOne({
      where: { id: clientId }
    })) as any
    const contractor: IProfile | null = (await Profile.findOne({
    ...
    if (clientBalance < price) {
      return res.status(422).send('Error: not enough balance')
    }

    // TODO: Put this code inside a transaction...
    const numberJobsUpdated = await Job.update(
      { paid: true },
      { where: { id: jobId } }
    )
    ...
    await Profile.update(
      { balance: contractorBalance + price },
      { where: { id: contractorId } }
    )
